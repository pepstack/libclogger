...
sb8 file_seek(filehandle_t hf, sb8 distance, int fseekpos)
{
    ...
    if (SetFilePointerEx(hf, li, &l2, fseekpos)) {
        // returns offset bytes from file' head
...
{
    // save file current pos relative to SEEK_SET
    sb8 offset0 = file_seek(hf, 0, SEEK_CUR);
    if (offset0 != -1) {
        // move pos to file end so we get file size
        sb8 offset2 = file_seek(hf, 0, SEEK_END);
        if (offset2 != -1) {
            // restore file pos relative to SEEK_SET
            if (file_seek(hf, offset0, SEEK_SET) != -1) {
                return offset2;
            }
        }
    }
...
sb8 file_seek(filehandle_t hf, sb8 distance, int fseekpos)
{
    // returns offset bytes from file head: SEEK_SET
    #ifdef __CYGWIN__
        /* warning: cygwin */
...
sb8 file_size(filehandle_t hf)
{
    sb8 offset0 = file_seek(hf, 0, SEEK_CUR);
    if (offset0 != -1) {
        // move to file END and get file size
        sb8 offset2 = file_seek(hf, 0, SEEK_END);
        if (offset2 != -1) {
